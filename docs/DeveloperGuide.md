...
**Note:** An alternative (arguably, a more OOP) model is given below. It has a `Group` list in the `AddressBook`, which `Person` references. This allows `AddressBook` to only require one `Group` object per unique tag, instead of each `Person` needing their own `Tag` objects. It also includes hiding the `Remark` in the `Schedule` class to ensure more OOP-ness.<br>
